...
{
    return cursor_reach_page_end(c) && get_page_num(cursor_page(c)) == get_page_cnt(c->pager) - 1;
}
...
void cursor_next(Cursor* c)
{
    ...
    if (cursor_reach_page_end(c) && !cursor_is_end(c)) {
        c->page = get_page(c->table, c->pager, get_page_num(cursor_page(c)) + 1);
        c->page_dir_num = 0;
    }
...
{
    c->page = get_page(c->table, c->pager, 0);
    c->page_dir_num = 0;
}